...
namespace CouplesBillManagerAPI.Helpers
{
  public class AuthHelper
  {
    ...
    public bool SetPassword(UserLoginDTO userForSetPassword)
    {
      ...
      sqlParametersUserExist.Add("@EmailParam", userForSetPassword.Email, DbType.String);
      string sqlAddAuth = "";
      if (_dapper.LoadDataWithParameters<string>(sqlCheckUserExist, sqlParametersUserExist).Count() == 0)
      {
        sqlAddAuth = $@"INSERT INTO Auth (Email, PasswordHash, PasswordSalt)
                    VALUES (@EmailParam, @PasswordHashParam, @PasswordSaltParam)";
      }
      else
      {
        sqlAddAuth = $@"UPDATE Auth SET PasswordHash = @PasswordHashParam, PasswordSalt = @PasswordSaltParam
